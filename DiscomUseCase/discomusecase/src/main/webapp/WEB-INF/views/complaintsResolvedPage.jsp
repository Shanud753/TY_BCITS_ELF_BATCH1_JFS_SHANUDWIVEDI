...
<%
  ...
  <tbody id="myTable">
    ...
    %>
  <form action="./sendResponse" method="get" >
  <input  type="text" name ="rrNumber" value="<%= supportBean.getSupportBeanCustPK().getRrNumber() %>" hidden="true" >
   <input  type="text" name ="date" value="<%= supportBean.getSupportBeanCustPK().getDate() %>" hidden="true" >
  	<tr>
      <th scope="row"><%= i %> </th>
       ...
       <td><strong><%= supportBean.getRequest() %> </strong></td>
        <td><strong><textarea class="form-Control" rows="2" id="comment" name="response"> </textarea></strong></td>
      <td><button type="submit" class="btn btn-primary">Click Here to Send</button></td> 
    </tr>
     </form>
    <% i++; }}%>
   		</tbody>
